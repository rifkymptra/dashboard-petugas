...
export async function GET(request: Request) {
  ...
  try {
    ...
    // --- FITUR PELACAK RAHASIA (Mengecek berapa data yang 100% selesai) ---
    let resolvedCount = 0;
    finalData.forEach(item => {
      // Mengecek apakah SEMUA anomali di dalam baris tersebut bernilai "1"
      const allResolved = item.daftar_anomali.every((a: any) => a.nilai === '1');
      if (allResolved) resolvedCount++;
    });
    console.log(`[PELACAK SISTEM] Jumlah baris yang 100% SELESAI (hanya berisi nilai '1'): ${resolvedCount}`);
    // ----------------------------------------------------------------------
    ...
    return NextResponse.json({
      ...
      statistik: {
        ...
        jumlah_selesai_100_persen: resolvedCount // Dikembalikan juga di response API
      }
    });
